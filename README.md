...
> `Cargo.toml` 遵循三位 semver 同步为 `0.3.4`（省略 `d`）；完整四位 `0.3.4.1`
> 通过 `crf-viewer --version` 显示，git tag 发布使用 `v0.3.4.1`。
...
- **CI/CD**：GitHub Actions —— `ci.yml`（门禁 + 三平台测试/构建）、`release.yml`（`v*` tag 三平台发布）；工具链锁定见 `rust-toolchain.toml`
