...
**已完成（2026-09-02，第一轮：消除重复 RCT 与全帧克隆）**：

1. **路径 G 重复全帧 RCT 消除**：`encode_sequence` 顶层原先无条件对全部帧
   做 `rct_forward` 并持有整份 `encode_frames` 副本——但该副本只被路径 C
   读取，路径 G（推荐路径、bench 主路径）完全不用（它在首帧/差分帧阶段内部
   独立做 RCT）。现延迟到路径 C 分支内懒计算：路径 G 的 `encode.rct` 阶段
   从 telemetry 消失（原 ~98ms/序列），并释放一整份全帧 RCT 副本的内存
   （组 1000 约 313MB），直接缓解 BATCH_MEM_LIMIT（1.5GB）下大图组峰值。

2. **原地 RCT 消除 to_vec 全帧克隆**：新增 `color::rct::rct_forward_in_place`
   （与 `rct_forward` 逐位一致，仅省中间 `pixels.to_vec()`）。首帧、路径 G
   差分帧、流式差分帧三处均由「分配新缓冲 + 变换」改为「独占缓冲原地改写」。
   每帧省一次全帧 memcpy（组 1000 单帧 ~22MB，13 差分帧 + 首帧）。

**收益与定性**：字节 11,090,743 逐字节一致，159 单测通过。这是**内存优先**
的 P1 项（时间收益约 1~2%，低于 3% 停止门槛；但消除了纯浪费的全帧 RCT +
...
**planar 子平面并行化尝试（2026-09-06，结论：嵌套过度订阅，已回退）**：

针对 P1c 遗留的「planar 620ms 受 `preferred_sub` 字节依赖约束」，
尝试打破串行依赖：去掉子平面间历史引导（`preferred_sub`）+ 3 子平面
`par_iter` 并行。同时段 bench（组 1000 无损路径 G）对比：

| 指标 | 串行基线 | 并行化 | 变化 |
|---|---:|---:|---:|
| bytes | 11,090,743 | 11,090,743 | 逐字节一致（无损下 planar 从不胜出）|
| planar mean | 1702ms | 1820ms | **+7%（变慢）** |
| encode p50 | 7944ms | 8012ms | +0.9%（噪声内持平）|

**根因**：嵌套并行过度订阅。planar 子平面内部已 rayon 并行（SATD 8 候选
`par_iter`、banded 条带并行），外层再并行 3 子平面使它们从「各自独占
16 线程」降为「瓜分 16 线程（~5 线程/子平面）」，内层并行度下降，抵消并
反超外层收益——与 §4.3「禁止 Rayon 嵌套并行」及 P1 第二轮「嵌套并行非
瓶颈、收紧会牺牲内层收益」结论一致。

**结论**：字节透明（去掉历史引导不影响无损产物，bytes 逐字节一致），
但速度无收益（略慢），故 `git revert` 回退（commit d7739b9），恢复
「串行 3 子平面 + 内层并行」。planar 620ms 的加速仍留待 P2 SIMD，或
「外层并行 + 内层禁用并行」的线程预算重构（当前不值得为负收益引入该复杂度）。

**重新评估（2026-09-07，未实施，见 optimization-review.md §36）**：P2 的
components==1 预测 SIMD 已落地但端到端 ≈0（内存带宽瓶颈，§33）；「外层并行 +
内层禁用并行」经评估收益约 −8~16% 端到端（内层并行加权减速比 ×2~3 与外层 ×3
对冲，仅凭 P1c 三点外推），且有损路径去掉 `preferred_sub` 的字节风险未消除、
线程池隔离复杂度高——本轮停止循环，留待后续专项性能轮统一处理（含 components==3
预测 SIMD 与「一次遍历合并 8 候选」）。

**✅ 已探针评估（2026-09-10，见 optimization-review.md §51）——不实施**：
`--probe-planar-parallel` 实测内层减速比仅 1.5~2.2×（非本节外推的 ×2~3），planar
阶段收益 +25%（优于本节 8~16% 估算），但 planar 占 encode p50 仅 15.2%，端到端
**+3.8%**（< 本规划 §4.6 的 10% 门槛）；内存峰值无显著惩罚（468.7 vs 452.6 MB）；
生产路径去 `preferred_sub` 的有损字节非透明（q90 +0.42% 劣化 / q75 −0.87% 改善）。
收益/风险比不划算，**证伪不实施**，探针与 `CRF_PLANAR_NO_PREF` 钩子保留为回归锚点。
...
**✅ P5~P7 一并关闭（依赖 P3/P4 成立）**：§P5（AMD HIP/ROCm + Vulkan/wgpu）、
§P6（GPU 扩大 RDO）、§P7（GPU 发布与维护）均以「P3~P5 证明传输/确定性可控」为
前提；P3/P4 已证伪（GPU 端到端 0.32~0.35×），前提不成立，故 P5~P7 一并关闭为
「依赖已证伪项、不实施」。未来若出现可摊销传输的超大图/超长序列批量场景，可整体复评。
