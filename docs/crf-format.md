...
**接入状态（v1.14）**：已作为 adaptive 候选竞争（frame_type=8，仅 3 分量
+ `dct_worth` 预筛时参与），decoder 分支已对称实现。三平面独立量化
（Y 用亮度 step，Co/Cg 用 chroma_step）。PNG1000 当前竞争未胜出
（planar 仍最优）——探针仅测 Y 平面的优势在整帧三平面竞争中不成立；
该候选的收益场景是与 planar 互补的内容类型分布。
