...
| **参考结构图 MST/深度约束森林** | §63 收益探针 | 收益不稳定否决：21 组仅 4~5 组达标（≥3%）；真实大差分组无收益（30-1-36 **+30.6%**、30-3-81 −0.7%），2 系列 13 组全未达标；SAD 边权与字节口径偏差（30-1-36 SAD −7.1% 但字节 +30.6%）；达标组深度 7~8（随机访问从深度 1 退化）；2 帧组恒等价。叠加 O(N²) 边权 + 破坏式帧头格式变更，不实施；探针 `--probe-ref-graph` 保留为回归锚点 |
| **位洗牌 + 零消除掩码（LICO, DCC 2024）** | §64 收益探针 | 位洗牌负增益否决：6 组全部 +5.4%~+71.7%（无达标）；根因——CRF 残差 i32 的 4 字节 word 恰等于一个残差，ZERE_4 直接消除整零残差，位洗牌按位平面重排破坏 word 对齐致零检测失效；LICO 位洗牌针对 8 位元素、CRF 残差 32 位属场景错配；LICO 无熵编码，整体比 CRF 现有编码 +256%~+728%。不实施；探针 `--probe-bit-shuffle` 保留为回归锚点 |
| **JXL 自校正/加权预测器（weighted::State）** | §65 收益探针 | 无显著增益否决：6 组最好 −2.0%、最差 **+24.8%**（3/6 变差）；CRF 残差是 RCT 域（已去相关），MED 平面外推已适配，JXL WP 的 4 子预测器针对未去相关像素通道设计，属场景错配；CRF adaptive 比 WP+RLE 再优 3.8%~19.9%。算法对照 libjxl `context_predict.h`（commit b5def9fb），单元测试锁定往返。不实施；探针 `--probe-self-correcting` 保留为回归锚点 |
| **JXL Squeeze 类 Haar 小波** | §66/§67 收益探针 | 净收益否决：§66 显示 squeeze 降低残差熵（RLE 口径 −12%~−54.7%，首个正收益信号）；但 §67 组合探针显示 squeeze 子带接 CRF 强编码器后比生产路径差 **+10.2%~+25.3%**（6/6 组）——子带已被小波去相关、空间预测失效、CABAC 效率下降 + 子带固定开销；分离分量本身也比交织差 +1.9%~+6.0%。不实施；探针 `--probe-squeeze`/`--probe-squeeze-adaptive` 保留为回归锚点 |
| **MA 树深度可配 + 真实字节 A/B** | §69 收益探针 | 不达门槛：条件熵 −4~−4.9%（§68）仅兑现为真实字节 **−0.3%~−1.2%**（最高熵组 30-1-36 −1.22%，采样 5 帧），6 组全部 <3%；根因——CABAC 自适应概率已捕获大部分上下文相关性 + 深度 6 树头 ≤380B（vs ≤75B）+ MA 常非整帧胜出 + d5 叶数饱和；默认 depth=3 维持（字节透明由 anchor 锁定），env `CRF_MA_MAX_DEPTH` 可配能力与探针 `--probe-ma-depth-ab` 保留为回归锚点 |
| **MA 树训练参数第二杠杆（gain/阈值）** | §70 收益探针 | **显著正收益，建议实施**：放宽训练参数（`gain=0.001 · log 阈值集 1..255 · depth=6`）真实字节 6 组中 5 组 ≥3% 门槛（2000 **−12.72%** / 2-12-4 −5.18% / 1000 −4.91% / 2-15-4 −3.16% / 10-1-10 −3.02%，30-1-36 −2.39% 采样），远优于深度杠杆（§69 −0.3~−1.2%）；主杠杆是阈值密度（>30 尾区无区分度）。默认参数可配（env `CRF_MA_MIN_GAIN`/`CRF_MA_THRESHOLDS`，默认逐位透明），探针 `--probe-ma-train-ab` 保留；实施需全组扫描 + 重标定基线 + 单调不劣化保障 |
...
## 八、2026-09-10 续：外部建议评审与超大组优化

> 本会话（optimization-review §51~§62）新增。**绝大多数方向经探针证伪或低于门槛**，
> 少数正面项已实施。
...
| 项 | 记录 | 结论 |
|---|---|---|
| planar 子平面并行化 | §51 | 端到端 +3.8% < 10%，有损字节非透明 |
| P2 剩余三项（SATD 合并 / 预测 SIMD / DCT SIMD） | §52 | 均低于门槛 |
| 首帧/变换域四项（P3.6/P3.7/P6 预测 SIMD/内存池） | §53 | 载体 frame_type=8 零胜出 |
| 压缩算法探索清单（§2~§5 多数候选） | §54 | 多数已落地 / 已证伪 |
| GPU 路线 P3~P7 | §55 | 端到端 0.32~0.35× |
| 外部建议（GPU/type8 过时判断纠正） | §56 | 收敛为少数新候选 |
| 调色板 MTF | §57 | palette 不参与胜出（B/A2 +1165%） |
| JPEG-XL 抖动调色板 | §58 | PSNR 仅 18~39dB（<40） |
| DCT i16 打包 SIMD | §59 | DCT 阶段仅占 encode 0.6% |

### 实施（正面）

| 项 | 版本 | 收益 |
|---|---|---|
| palette 候选级 Fast-Fail（方案 B） | v0.3.3.14 | 字节透明 + encode −1.8% |
| 专家面板 schema 补全（18→35 字段） | v0.3.3.15 | 接口收尾（§61） |
| batch 移除 50 帧硬限制 + 自动切分 | v0.3.3.16 | 支持任意帧数（有界并发） |
| 分批加载并行编码（batched） | v0.3.3.17 | 30-3-81 支持（§62） |
| 融合差分+RCT（`sub_rct_forward`） | v0.3.3.19 | 大图 −9~16%（省全帧往返） |
| 帧级并行度增大（内存 4GB + 核数上限） | v0.3.3.20 | 30-3-81 −14~15% |

### 超大组（30-3-81，81 帧 3826×5412）最终性能

| 模式 | 体积 | 时间 | vs 串行 |
|---|---:|---:|---:|
| adaptive（无损） | 570.59 MB | 461.7 s | ~2.8× |
| q90 | 323.66 MB | 666.4 s | — |

> 字节全程逐字节不变；工具：`--probe-group-bench`、`CRF_BATCH_FRAMES`、
> `CRF_BATCH_MEM_LIMIT`、`CRF_STREAMING=1`（串行保底）。
