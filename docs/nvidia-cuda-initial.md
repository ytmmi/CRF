...
- `NvidiaCudaBackend`：通过公共 `BackendKernel` 暴露窄适配层；启用 feature 后提供 `diff_i32`、`rct_forward` 与 `rct_inverse`，通过旁路 `crf_cuda.dll` 调用 CUDA Driver API 并执行 PTX（diff + YCoCg-R 正/逆变换三个 kernel，共享同一 module/context）。
- CUDA 运行时会缓存 Driver、context、module（kernel function 按名惰性获取）；每次调用仅分配/释放输入输出缓冲区，降低重复初始化开销，并在会话销毁时安全释放 CUDA 资源。

默认 feature 为 `nvidia-cuda`，统一 `backend::ops::sub_i32`、`backend::ops::rct_forward` 与 `backend::ops::rct_inverse` 会在首次达到 1M 元素（rct 为 3 分量交织，即约 349K 像素）时检查设备并优先使用 GPU；小输入、无 NVIDIA 环境、驱动异常或 kernel 失败会自动切换到 CPU。GPU 失败状态会被记忆，避免每次调用重复探测。
...
2. RCT 正/逆变换已接入默认编码/解码路径（`backend::ops::rct_forward` → `crf_cuda_rct_forward`、`backend::ops::rct_inverse` → `crf_cuda_rct_inverse`）；下一步将 diff/RCT 扩展为批量 SoA 布局，再实现固定 tile DCT；采用异步上传、双缓冲和一次性回读。
