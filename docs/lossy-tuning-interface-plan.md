...
> **项目现状（2026-09-07）**：本项目为**纯 CLI**，仓库没有 React/Tauri 前端源码。
> UI 落地点是 `--dump-expert-schema` / `lossy_expert_schema_json()` 导出的专家面板
> schema 与解析 API——未来前端不得复制另一份默认和校验逻辑。以下为未来 UI 的面板
> 规划，不代表已实现。
