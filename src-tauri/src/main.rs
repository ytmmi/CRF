...
pub const APP_VERSION: &str = "0.4.0.0";
...
fn main() {
    ...
    if args.len() > 1 && args[1] == "--probe-cabac-share" {
        // cabac/MA 变体胜出率诊断（frame_type 分布 + cabac 内三变体 + 差距）
        let dir = if args.len() > 2 {
            args[2].clone()
        } else {
            String::from(r"E:\CRF\test\png")
        };
        if let Err(e) = crf::performance::probe_cabac_share::run(&dir) {
            eprintln!("Probe error: {e}");
            std::process::exit(1);
        }
        return;
    }
    if args.len() > 1 && args[1] == "--probe-banded-cabac" {
        // banded 条带级熵编码升级诊断（条带残差 RLE+Golomb vs CABAC）
        let dir = if args.len() > 2 {
            args[2].clone()
        } else {
            String::from(r"E:\CRF\test\png")
        };
        if let Err(e) = crf::performance::probe_banded_cabac::run(&dir) {
            eprintln!("Probe error: {e}");
            std::process::exit(1);
        }
        return;
