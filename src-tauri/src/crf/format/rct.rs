//! 可逆色彩空间变换（Reversible Color Transform）
//!
//! 参考 AV1 / HEVC Range Extensions / JPEG-XL 无损模式的标准做法：
//! RGB 三通道高度相关，变换到亮度/色度分离的 YCoCg-R 域后
//! 各分量残差能量更集中、分布更尖峰，显著降低后续熵编码的输出大小。
//! 变换为纯整数运算，数学上严格可逆（无损）。
...
pub fn rct_forward(pixels: &[i32], components: usize) -> CrfResult<Vec<i32>> {
    ...
    super::simd::rct_forward_interleaved(&mut out);
    Ok(out)
}
...
pub fn rct_inverse(pixels: &[i32], components: usize) -> CrfResult<Vec<i32>> {
    ...
    super::simd::rct_inverse_interleaved(&mut out);
    Ok(out)
}
