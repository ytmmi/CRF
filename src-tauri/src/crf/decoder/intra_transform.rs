...
pub fn decode_intra_transform(
    ...
    if components != 3 || data.is_empty() {
        return Err(CrfError::InvalidCodingParams("intra_transform: bad params".into()));
    }
    ...
    for _ in 0..3 {
        if offset + 4 > data.len() {
            return Err(CrfError::InsufficientData { expected: offset + 4, actual: data.len() });
        }
        let plen = u32::from_le_bytes(data[offset..offset + 4].try_into().unwrap()) as usize;
        offset += 4;
        if offset + plen > data.len() {
            return Err(CrfError::InsufficientData { expected: offset + plen, actual: data.len() });
        }
...
fn decode_plane(data: &[u8], width: usize, height: usize, q: i32) -> CrfResult<Vec<i32>> {
    if data.len() < 4 {
        return Err(CrfError::InsufficientData { expected: 4, actual: data.len() });
    }
    let mode_len = u32::from_le_bytes(data[0..4].try_into().unwrap()) as usize;
    if 4 + mode_len > data.len() {
        return Err(CrfError::InsufficientData { expected: 4 + mode_len, actual: data.len() });
    }
    ...
    let modes = crate::crf::decoder::rle_cabac::decode_frame_rle_cabac(
        mode_data,
        0, // k 从载荷提取——当前模式流走 v3 payload [k][body]，k 在首字节
        block_count,
        Some(width / BLK),
    );

    // 解码系数流（CABAC）
    // TODO: CABAC 解码器需要对称的 RangeDecoder + 上下文
    // 当前占位：直接用 RLE+CABAC 的 decode_frame_rle_cabac 解码系数
    let coeffs = crate::crf::decoder::rle_cabac::decode_frame_rle_cabac(
        coeff_data,
        0,
        width * height, // 每像素一个系数（块铺平）
        None,
    );
    ...
    for y0 in (0..height).step_by(BLK) {
        for x0 in (0..width).step_by(BLK) {
            ...
            // 从系数流取 64 个值（zigzag 序）
            let mut qz = [0i32; 64];
            for i in 0..64 {
                qz[i] = coeffs.get(coeff_idx + i).copied().unwrap_or(0);
            }
            coeff_idx += 64;
    ...
    match mode {
        ...
        MODE_V => {
            ...
            for row in out.iter_mut() {
                for (v, t) in row.iter_mut().zip(tops.iter()) { *v = *t; }
            }
        }
        MODE_DC => {
            let mut sum = 0i64;
            let mut cnt = 0i64;
            if x0 > 0 { for by in 0..BLK { sum += recon_at(recon, w, h, x0 as isize - 1, (y0 + by) as isize) as i64; cnt += 1; } }
            if y0 > 0 { for bx in 0..BLK { sum += recon_at(recon, w, h, (x0 + bx) as isize, y0 as isize - 1) as i64; cnt += 1; } }
            let dc = if cnt > 0 { (sum / cnt) as i32 } else { 0 };
            for row in &mut out { for v in row.iter_mut() { *v = dc; } }
        }
        _ => {
            ...
            for (by, row) in out.iter_mut().enumerate() {
                ...
                for (bx, v) in row.iter_mut().enumerate() {
                    ...
                    else { *v = (a + b - c).clamp(a.min(b).min(c), a.max(b).max(c)); }
                }
            }
        }
    }
