//! Zigzag 扫描与符号编码（原 format/zigzag.rs，P4 架构迁移）
//!
//! 规划文档 §3.7 / `entropy/scan.rs`。包含：
//! - ZIGZAG_4X4 / ZIGZAG_8X8：扫描顺序表；
//! - zigzag_scan / zigzag_inverse：块扫描正逆变换；
//! - zigzag_encode / zigzag_decode：有符号↔无符号 zigzag 映射。
//!
//! **迁移说明（P4）**：本模块原位于 `format/zigzag.rs`，现迁移到
//! `core/entropy/scan`。`format/zigzag.rs` 保留为 `pub use` 转发层。
